...
class DatabaseConfig(BaseModel):
    ...
    path: Optional[str] = Field(default=None, description="Database file path for SQLite")
...
class LoggingConfig(BaseModel):
    ...
    file: Optional[str] = Field(default="app.log", description="Log file path")
...
class QuickenConfig(BaseModel):
    ...
    executable_path: Optional[str] = Field(default=None, description="Quicken executable path")
    data_file: Optional[str] = Field(default=None, description="Quicken data file path")
...
class Config(BaseModel):
    ...
    input_statements_folder: Optional[str] = Field(
        default=None, 
        description="Input folder containing unsorted statements (from INPUT_STATEMENTS_FOLDER env var)"
    )
    
    # Target folder for organized statements (from TARGET_STATEMENTS_FOLDER env var)
    target_statements_folder: Optional[str] = Field(
        default=None, 
        description="Target folder for organized statements (from TARGET_STATEMENTS_FOLDER env var)"
    )
    
    # Financial institution specific settings
    institutions: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Institution-specific configuration"
    )
