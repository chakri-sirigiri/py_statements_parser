...
def test_invalid_financial_institution(runner):
    ...
    result = runner.invoke(
        app, ["--financial-institution", "invalid", "--feature", "rename-file", "--statements-folder", "test"]
    )
...
def test_invalid_feature(runner):
    ...
    result = runner.invoke(
        app, ["--financial-institution", "ipay", "--feature", "invalid-feature", "--statements-folder", "test"]
    )
    assert result.exit_code == 1
...
def test_missing_statements_folder(runner):
    ...
    result = runner.invoke(app, ["--financial-institution", "ipay", "--feature", "rename-file"])
    assert result.exit_code == 1
